...
if ( ! class_exists( 'NGIP_Reg_Script' ) ) {
		...
		public function register( $dispatch = null ) {
			if ( $this->handle && $this->src && ! wp_script_is( $this->handle, 'registered' ) ) {
				if ( self::WP_SCRIPT === $this->deps ) {
					...
					$path = path_join( dirname( ngip()->get_main_file() ), "assets/js/{$dir}/{$file}" );

					if ( file_exists( $path ) && is_readable( $path ) ) {
						$info = include $path;

						$this->src       = plugins_url( "assets/js/{$this->src}", ngip()->get_main_file() );
						$this->deps      = $info['dependencies'] ?? [];
						$this->ver       = $info['version'] ?? ngip()->get_version();
						$this->in_footer = true;
					}
				}
