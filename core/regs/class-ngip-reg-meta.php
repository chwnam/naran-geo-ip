			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new NGIP_Reg_Meta( $object_type, $meta_key, $args );
				}
			...
			if ( $this->object_type && $this->get_key() ) {
				try {
					...
					$error->add(
						...
						sprintf(
							...
							ngip_format_callback( $this->args['sanitize_callback'] )
						)
					);
					wp_die( $error );
				}
				...
				try {
					if ( $this->args['auth_callback'] ) {
						$this->args['auth_callback'] = ngip_parse_callback( $this->args['auth_callback'] );
					}
				} catch ( NGIP_Callback_Exception $e ) {
					$error = new WP_Error();
					$error->add(
						...
						sprintf(
							...
							ngip_format_callback( $this->args['auth_callback'] )
						)
					);
					wp_die( $error );
				}
