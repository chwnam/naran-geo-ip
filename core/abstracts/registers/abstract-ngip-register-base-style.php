	...
	abstract class NGIP_Register_Base_Style implements NGIP_Register {
		...
		public function register() {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof NGIP_Reg_Style ) {
					$item->register();
				}
			}
		}
		...
		protected function src_helper( string $rel_path, bool $replace_min = true ): string {
			...
			if ( ngip_script_debug() && $replace_min && substr( $rel_path, - 8 ) === '.min.css' ) {
				$rel_path = substr( $rel_path, 0, strlen( $rel_path ) - 8 ) . '.css';
			}

			return plugin_dir_url( ngip()->get_main_file() ) . 'assets/css/' . $rel_path;
		}
	}
}
